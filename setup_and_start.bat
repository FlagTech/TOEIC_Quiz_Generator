...
start "TOEIC Quiz Generator - Backend" cmd /k "cd /d "%CD%" && uv run uvicorn backend.main:app --host 127.0.0.1 --port %BACKEND_PORT% --reload"
...
echo ====================================
echo.
echo   Close each service window (Backend/Frontend)
echo   or press Ctrl+C in this window
echo ====================================
echo.

REM Keep this window open and wait for Ctrl+C
:WAIT_LOOP
